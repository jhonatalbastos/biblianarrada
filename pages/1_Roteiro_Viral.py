...
# Inicializa o cliente Groq (certifique-se de ter st.secrets["GROQ_API_KEY"])
...
def gerar_conteudo_ia(texto_original, referencia):
    ...
    with st.spinner('🤖 A IA está lendo o Evangelho, refletindo e orando... Por favor, aguarde.'):
        ...
        # --- GERAÇÃO DE PROMPTS DE IMAGEM (Interno) ---
        prompts_img = {
            ...
            "bloco_2": f"A photorealistic image of Jesus Christ (traditional appearance with robes) sitting in a modern, busy everyday setting (like a coffee shop, a subway station, or a busy park). Action: He is having a friendly, engaging conversation with a modern-day person. Context: They are discussing the biblical theme: '{texto_original}'. Style: Candid photography, depth of field, natural lighting, realistic interactions.",
            "bloco_3": f"A photorealistic image of Jesus Christ (traditional appearance) walking or standing with a modern person in a public urban space (like a street, waiting for a bus, or an office break room). Action: Jesus is gesturing kindly, offering advice or comfort, like a mentor. Context: Applying the lesson of this text to real life: '{texto_original}'. Style: Warm atmosphere, urban photography style, 4k resolution.",
            "bloco_4": f"A serene, photorealistic image of Jesus Christ and a modern person in a quiet, calm location (like a peaceful living room or a quiet garden corner). Action: They are praying together, perhaps with eyes closed or hands clasped. Atmosphere: Spiritual, peaceful, soft divine lighting, intimate and comforting. Context: Based on the spirituality of: '{texto_original}'."
        }
...
with col_dir:
    ...
    with st.form("form_roteiro"):
        ...
        if submitted:
            ...
            # Salva os prompts de imagem (se foram gerados agora ou mantem os antigos)
            if 'temp_p_imgs' in st.session_state:
                 progresso['prompts_imagem'] = st.session_state['temp_p_imgs']
            
            # Cria um texto completo concatenado para referência futura se necessário
...
with col_nav_3:
    if progresso.get('roteiro'):
        if st.button("Próximo: Criar Imagens (IA) ➡️", type="primary", use_container_width=True):
            st.switch_page("pages/2_Imagens.py") # Assumindo que você renomeará ou ajustará a próx página
    else:
